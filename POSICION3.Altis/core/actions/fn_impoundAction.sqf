...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	[[0,format[localize "STR_NOTF_BeingImpounded123",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	if(player distance _vehicle > 10) exitWith {hint localize "STR_NOTF_ImpoundingCancelled123"; life_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		hint format[localize "STR_NOTF_Impounded123",_type,_price];
		[[0,format[localize "STR_NOTF_HasImpounded123",profileName,(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		//life_atmcash = life_atmcash + _price;
	}
		else
	{
		hint localize "STR_NOTF_ImpoundingCancelled123";
	};
};
