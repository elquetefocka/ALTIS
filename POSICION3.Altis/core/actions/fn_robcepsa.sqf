...
if (_cops < 2) exitWith {
hint format ["La Iglesia no puede ser robado por que hay %1 policia online.", _cops];};
...
hint format ["Robando recaudacion de la iglesia!Espera %1 sec.",_timer];
...
[[2,"La Iglesia esta siendo atracada!!"],"life_fnc_broadcast",east,false] spawn life_fnc_MP;
[[2,"La Iglesia esta siendo atracada!!"],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		...
		if (!alive _robber) exitwith
			{
				...
				_shop switchMove "";
				_action = _shop addAction["Robar los fondos",life_fnc_robcepsa];
			};
...
_action = _shop addAction["Robar al Cura",life_fnc_robcepsa];//Adding action for the robbery
