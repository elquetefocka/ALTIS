...
if (_cops < 2) exitWith {
hint format ["La Repsol no puede ser robado por que no hay %1 policias online.", _cops];};
...
[[2,"La Repsol esta siendo atracada!!"],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
[[2,"La Repsol esta siendo atracada!!"],"life_fnc_broadcast",east,false] spawn life_fnc_MP;
		...
		if (!alive _robber) exitwith
			{
				...
				_shop switchMove "";
				_action = _shop addAction["Robar al gasolinero",life_fnc_robrepsol];
			};
...
_action = _shop addAction["Robar al gasolinero",life_fnc_robrepsol];//Adding action for the robbery
