...
if(life_inv_kidney >= 2) exitWith {hint "No es posible que necesite que muchos órganos!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hint "no tiene espacio para almacenar cualquier órgano!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
