...
if(!isNil {admin_query_ip}) exitWith {hint "Ya pidas a un jugador."};
_text = _display displayCtrl 2903;
_info = lbData[2902,lbCurSel (2902)];
_info = call compile format["%1", _info];
if(isNil "_info") exitWith {_text ctrlSetText "Jugador ya no existe?";};
if(isNull _info) exitWith {_text ctrlSetText "Jugador ya no existe?";};
