...
_Btn1 buttonSetAction "
		...
		if(license_civ_bountyh && (side life_pInact_curTarget != civilian)) exitWith {hint 'Vigilanties can only restrain civilians.';};
		
		if(([false,'ziptie',1] call life_fnc_handleInv)) then
		{
			life_pInact_curTarget setVariable['restrained',true,true];
			[[player], 'life_fnc_restrainciv', life_pInact_curTarget, false] spawn life_fnc_MP;
...
_Btn6 buttonSetAction "if(player distance life_pInact_curTarget > 4) exitWith {hint 'You are too far away.'}; [life_pInact_curTarget] call life_fnc_arrestAction; closeDialog 0;";
...
if(currentWeapon player == "" || currentWeapon player in life_fake_weapons) then {
	_Btn1 ctrlEnable false;
	_Btn2 ctrlEnable false;
	if(!(_curTarget getVariable["Escorting",false])) then {
		_Btn3 ctrlEnable false;
