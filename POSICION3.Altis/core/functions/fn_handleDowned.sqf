...
if (!life_isdowned) then {
	...
	[[0,format["%1 ha sido derribado por %2.",name player, name _source]],"life_fnc_broadcast",true,false] spawn life_fnc_MP; //testing
	life_isdowned = true;
		...
		if (alive player) then {
			if (!(player getVariable ["restrained",false])) then{
				if (!(_inVehicle)) then{
					...
					if (_time == 15) then {_downed = false;}; //testing
				};
