...
if(!([_uid] call life_fnc_isUIDActive)) exitWith {hint "Esta persona no está en línea allí para que no se puede atacar a su casa!"};
_houseInv = _house getVariable ["Trunk",[[],0]];
if(_houseInv isEqualTo [[],0]) exitWith {hint "No hay nada en esta casa."};
life_action_inUse = true;

//Setup the progress bar
disableSerialization;
_title = "Registrando Casa....";
...
5 cutText ["","PLAIN"];
if(player distance _house > 13) exitWith {life_action_inUse = false; titleText["Te fuiste demasiado lejos de la casa!","PLAIN"]};
...
if(_value > 0) then {
	[[0,format["Una casa fue asaltada e incauto $%1 en drogas.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
