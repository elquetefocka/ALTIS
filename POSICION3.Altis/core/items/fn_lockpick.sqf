...
if(!_isVehicle) then {
	...
	_dice = random(100);
	if(_dice < 30) then {
		titleText["Ahora tu tienes la llaves de este vehiculo.","PLAIN"];
		...
		[[getPlayerUID player,profileName,"215"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
 		[[0,format["%1 fue visto intentando robar un coche.",profileName]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		titleText["La ganzua se ha roto.","PLAIN"];
	};
};