...
switch (true) do
{
	...
	case (player distance (getMarkerPos "rubi_1") < 50): {_mine = "rubiu"; _val = 2;};
	default {_mine = "";};
};
//Mine check
if(_mine == "") exitWith {hint "No estas cerca de una mina"};
if(vehicle player != player) exitWith {hint "No puedes picar desde dentro de el coche!";};

_diff = [_mine,_val,life_carryWeight,life_maxWeight] call life_fnc_calWeightDiff;
if(_diff == 0) exitWith {hint "Inventario lleno."};
...
if(([true,_mine,_diff] call life_fnc_handleInv)) then
{
	...
	titleText[format["Has picado %2 %1",_itemName,_diff],"PLAIN"];
};
