...
_c_arr = [_veh] call life_fnc_vehiclerepaintCfg;
_id = switch(playerSide) do {case civilian: {"civ"}; case west:{"cop"}; case west:{"copeast"}; default {"civ"};};
_colorcost = 20000;
for "_i" from 0 to (count _c_arr)-1 do
{
if((_c_arr select _i) select 1 == _id) then
			{
_text = [_veh,_i] call life_fnc_vehiclerepaintStr;
