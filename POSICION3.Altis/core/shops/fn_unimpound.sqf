...
if(_veh distance impound_obj < 50) then
{
	...
	if(count _check > 0) exitWith {hint "En la actualidad existe un coche allí.";};
	if(_price > life_cash) then
	{
		_price = _price + 200;
		if(_price > life_atmcash) exitWith {hint "No tiene suficiente dinero encima o en su banco para recuperar su vehiculo."};
	...
	hint format["Has recuperado tu %1 por $%3",_name,[_price] call life_fnc_numberText];
	detach _veh;
