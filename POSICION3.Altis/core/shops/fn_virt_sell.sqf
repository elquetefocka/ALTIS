...
if(!([_amount] call fnc_isnumber)) exitWith {hint "No has introducido un numero numero real";};
_amount = parseNumber (_amount);
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "No tienes ese o tantos artículos para vender!"};

_price = (_price * _amount);
_name = [_var] call life_fnc_vartostr;
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	hint format["Vendiste %1 %2 por $%3",_amount,_name,[_price] call life_fnc_numberText];
