...
if(playerSide == west OR playerSide == east) then {
	...
	_Btn8 ctrlSetText localize "STR_vInAct_Impound112";
	_Btn8 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_impoundAction112;";
...
if(!((player distance (getMarkerPos "deposito_1") < 30) OR  (player distance (getMarkerPos "deposito_2") < 30) OR (player distance (getMarkerPos "deposito_3") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30))) then 
{
